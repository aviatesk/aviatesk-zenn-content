...
2. **$L = <A, \sqcup, \sqcap>$**: $P$が取りうる抽象値の[lattice](https://en.wikipedia.org/wiki/Lattice_(order))
...
- $\sqcup, \sqcap$: それぞれ[joinとmeet](https://en.wikipedia.org/wiki/Join_and_meet)に対応する演算で、$A$に対して作用する
...
それでは早速 1.) プログラム $P$, 2.) lattice $L$, 3.) abstract semantic $![.!]$, 4.) プログラムの初期状態 $a_0$ のそれぞれを確認しましょう。
...
これらの順序関係を次のように定義します: $c_1 \le c_2$ iff a) $c_1 = c_2$, b) $c_1 = \bot$, c) $c_2 = \top$

$C$は次の画像のようなフラットなlatticeとなります:
![lattice](./data-flow-problem-20201025-assets/lattice.png)
...
![algorithm](./data-flow-problem-20201025-assets/alrogithm.png)

このアルゴリズムの直観的な理解としては以下のようになります:
1. プログラム自身がinstruction levelで(abstract semantic $![.!]$を通して)その抽象状態へ作用する
2. アルゴリズムは現在のinstruction $I$を示す現在のprogram counter $pc$と、作用が計算されるべき残りのinstruction達に対応する$pc$を保持するworking set $W$を更新しながら動作する
2. 現在のinstruction$I$の抽象状態の変化は、$I$が到達する可能性があるinstruction全てに伝播する
3. ただし、現在のinstruction$I$の抽象状態の変化は、**伝播する先の抽象状態を変化させる場合にのみ**伝播する

3.はアルゴリズム中の$I_{pc} = (\text{if} \psi \text{goto} l) ...$の箇所に相当します。これはつまり、このアルゴリズムは条件分岐の分岐先について両方の分岐を勘定するということです。
...
![tracing example](./data-flow-problem-20201025-assets/tracing-example.png)

最終的な状態$s_8$において、プログラム中にない`r`が定数`5`であるという情報が現れているのが分かると思います。
...
次に、各変数$X$から抽象値$C$へのmapとして抽象状態$A$を表現し、その順序関係$<$を定義します:
```julia
...
abstract semantic $![.!]$は、実際のJuliaのexecutionを用いて簡単に実装できます。
`Call`の`head` fieldから実際の演算メソッドは`getfield`を用いて復元できます。
```julia
...
ちょっと醜いですね。
ここでは、Juliaの強力なメタプログラミング機能を用いて、Juliaのsyntaxから今回data-flow problemターゲットとするプログラム$P$を生成するマクロを記述しましょう[^2]。
...
これは実は論文側の間違いなのですが、詳しくみてみましょう[^3]。
...
そうすると、左から11行目上から10行目で$s_7$の状態が$s_8$に条件$s_8 < s_7$が成り立たず、そのまま$W$が空となりアルゴリズムは停止します(i.e. 実装の`new < s[pc´+1]`が`false`となります)。

上の実装は論文に忠実に従っているので、以上で説明した挙動の通りにアルゴリズムが停止してしまいます。さぁ困った。

### 論文のアルゴリズムをデバッグ

論文のアルゴリズムの問題点は、端的に言えば、「伝播する先の抽象状態を変化させるかどうか」という判定を抽象状態同士の順序関係を用いて判定するとうまく状態が伝播しないということです。
つまり、この場合、$s_8$の前状態`s[pc´+1]`($\text{x}/1 \text{y}/2 \text{z}/3 \text{r}/\top$)と現状態`new = s[pc´]`($\text{x}/2 \text{y}/2 \text{z}/3 \text{r}/5$)の間には`new < s[pc´+1]`という順序関係は成立しませんが[^4]、`new`の状態を`s[pc´+1]`へ伝播させ、`s[pc´+1]`を新たな状態($\text{x}/\bot \text{y}/2 \text{z}/3 \text{r}/5$)へと更新したいわけです。
...
見事、$I_8$の状態$s_8$に該当する`Dict(:y => Const(2), :z => Const(3), :r => Const(5), :x => ⊥)`で、`r`が定数`Const(5)`であるという情報が見つけられていますね。


## Julia自身の型推論と比較してみる

先述したように、Juliaの型推論アルゴリズムは今回紹介した[Mohnen, Markus. “A Graph-Free Approach to Data-Flow Analysis.” _CC_ (2002).](https://www.semanticscholar.org/paper/A-Graph-Free-Approach-to-Data-Flow-Analysis-Mohnen/5ad8cb6b477793ffb5ec29dde89df6b82dbb6dba?p2df)で提案されたアルゴリズムに基づいています。となると、Juliaの型推論ルーチンが同様の修正を加えているのかどうか、`prog0`に対して正しく動くのかどうか気になるところです。

### まず試してみる

まずは`prog0`に対応するJulia programを作って、型推論を走らせてみましょう。Juliaの型推論はdata-flow analysisによりJuliaプログラムの型付けを行うことを目的としていますが、推論の精度を高めるために定数伝播も行うため、もしJuliaの型推論ルーチンが正しく動くならば推論の結果に`r`が定数`5`であるという情報が現れているはずです。
...
やはり少し醜いので、$P = I_0 ... I_n \in \text{Instr}$の記述方法から実際に実行可能なJuliaコードを生成するマクロ`@prog′`を定義しましょう:
```julia
...
quote
    begin
        $(Expr(:symboliclabel, Symbol("#1154###label0#1004")))
        var"#1166#x" = 1
    end
    begin
        $(Expr(:symboliclabel, Symbol("#1155###label1#1005")))
        var"#1163#y" = 2
    end
    begin
        $(Expr(:symboliclabel, Symbol("#1156###label2#1006")))
        var"#1164#z" = 3
    end
    begin
        $(Expr(:symboliclabel, Symbol("#1157###label3#1007")))
        $(Expr(:symbolicgoto, Symbol("#1158###label8#1012")))
    end
    begin
        $(Expr(:symboliclabel, Symbol("#1159###label4#1008")))
        var"#1165#r" = var"#1163#y" + var"#1164#z"
    end
    begin
        $(Expr(:symboliclabel, Symbol("#1160###label5#1009")))
        var"#1166#x" ≤ var"#1164#z" && $(Expr(:symbolicgoto, Symbol("#1161#
##label7#1011")))
    end
    begin
        $(Expr(:symboliclabel, Symbol("#1162###label6#1010")))
        var"#1165#r" = var"#1164#z" + var"#1163#y"
    end
    begin
        $(Expr(:symboliclabel, Symbol("#1161###label7#1011")))
        var"#1166#x" = var"#1166#x" + 1
    end
    begin
        $(Expr(:symboliclabel, Symbol("#1158###label8#1012")))
        var"#1166#x" < 10 && $(Expr(:symbolicgoto, Symbol("#1159###label4#1
008")))
    end
end
```
...
`8 ─ %15 = Core.tuple(x, y::Core.Const(2), z::Core.Const(3), r::Core.Const(5))::Core.PartialStruct(NTuple{4, Int64}, Any[Int64, Core.Const(2), Core.Const(3), Core.Const(5)])`という出力に現れているように、Juliaの型推論ルーチンは正しく`r`が定数(Julia compilerの表現としては`Core.Const(5)`)である情報を見つけることができていますね。

:::details 実行してみる
実行してみると、期待通り`r == 5`という結果が得られているのが確認できます:
```julia
@prog′ begin
    x = 1
    y = 2
...
Juliaの型推論ルーチンは最も抽象的な抽象値である`Any`を頂上とし、そこにいけば行くほど具体的な抽象値[^5] になるようなlatticeに対しで動作し、状態がlatticeの底から上に登るように状態を更新していくので[^6] 、この記事で実装したconstant folding propagation problemが使用する$\sqcap$ではなく$\sqcup$を用いているというわけです。

[^5]: 具体的な抽象値の例としては`Int`や、より具体的な`Core.Const(1)`などがあります。
...
- この記事の本体及び使用したコードは[https://github.com/aviatesk/aviatesk-zenn-content](https://github.com/aviatesk/aviatesk-zenn-content)で公開しています。タイポなど見つけた際は是非コメントあるいはPRをいただけると嬉しいです。
